...
public interface DepartmentRepository extends R2dbcRepository<Department, Long>, DepartmentRepositoryInternal {
    ...
    @Query("SELECT * FROM department entity WHERE entity.manager_id = :id")
    Flux<Department> findByManager(Long id);

    @Query("SELECT * FROM department entity WHERE entity.manager_id IS NULL")
    Flux<Department> findAllWhereManagerIsNull();
