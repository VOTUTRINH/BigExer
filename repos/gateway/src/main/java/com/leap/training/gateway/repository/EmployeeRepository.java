...
public interface EmployeeRepository extends R2dbcRepository<Employee, Long>, EmployeeRepositoryInternal {
    ...
    @Query("SELECT * FROM employee entity WHERE entity.job_id = :id")
    Flux<Employee> findByJob(Long id);

    @Query("SELECT * FROM employee entity WHERE entity.job_id IS NULL")
    Flux<Employee> findAllWhereJobIsNull();
